  ...
  <div class="table-responsive">
    <table class="table table-striped im-table">
      ...
      <?php if (!empty($email_templates)): foreach($email_templates as $t): ?>
        <tr data-id="<?= (int)$t->id ?>" data-name="<?= im_h($t->name) ?>" data-code="<?= im_h($t->code) ?>" data-subject="<?= im_h($t->subject) ?>" data-active="<?= isset($t->is_active) ? (int)$t->is_active : (isset($t->active)?(int)$t->active:1) ?>" data-content="<?= im_h($t->content) ?>">
          <td><?= (int)$t->id ?></td>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    var btn   = document.getElementById('im_btn_test_send');
    var input = document.getElementById('im_test_to');
    var box   = document.getElementById('im_test_result');
    ...
    btn.addEventListener('click', function(){
        ...
        box.innerHTML = '<div class="text-info"><i class="fa fa-spinner fa-spin"></i> Đang gửi...</div>';

        var xhr = new XMLHttpRequest();
        xhr.open("POST", "<?= admin_url('internship_management/internship_settings/test_smtp'); ?>", true);
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8");
            ...
            if(xhr.status >= 200 && xhr.status < 300){
                try{
                    ...
                    if(res.ok){
                        ...
                        box.innerHTML = '<div class="alert alert-danger">'+esc(res.message || 'Gửi thất bại')+'</div>';
                    }
                }catch(e){
                    box.innerHTML = '<div class="alert alert-danger">Phản hồi không hợp lệ</div>';
                }
            }else{
                box.innerHTML = '<div class="alert alert-danger">Lỗi request: '+xhr.status+'</div>';
            }
        };
