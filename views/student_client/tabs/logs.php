...
if (empty($logs) && $student_id > 0) {
    $candidates = [
        ...
        db_prefix().'internship_processing_logs',
    ...
    if ($log_table) {
        ...
        // detect relation field
        $fields = $CI->db->list_fields($log_table);
        if (in_array('student_id', $fields, true)) {
            $CI->db->where('l.student_id', $student_id);
        } elseif (in_array('rel_id', $fields, true)) {
            $CI->db->where('l.rel_id', $student_id);
            if (in_array('rel_type', $fields, true)) {
                $CI->db->where('l.rel_type', 'student_client');
            }
        } else {
            // fallback: try common id fields
            foreach (['client_id','userid','contact_id'] as $f) {
                if (in_array($f, $fields, true)) { $CI->db->where('l.'.$f, $student_id); break; }
            }
        }
